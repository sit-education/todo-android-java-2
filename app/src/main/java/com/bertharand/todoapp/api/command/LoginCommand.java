...
public class LoginCommand extends BaseCommand implements Callback<LoginResponse> {
    ...
    protected final void doExecute(Context context) {
        Call<LoginResponse> loginCall = ToDoApiService.getInstance().login(mLogin, mPassword);

        loginCall.enqueue(this);
    }

    @Override
    public final void onResponse(Response<LoginResponse> response, Retrofit retrofit) {
        if(response.isSuccess() && response.body()!= null) {
            notifySubscribers(new LoginSuccessEvent(response.body().getUserData()));
        } else {
